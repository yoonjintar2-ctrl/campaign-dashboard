/* ===== 13. 트렌드 리포트 — 모든 캠페인이 함께 쓰는 자료 게시판 (v66) =====
   ...
   · 파일은 한 개당 5MB 까지. 클라우드가 붙어 있으면 Supabase Storage,
     붙어 있지 않으면(데모) 이 브라우저 안에서만 도는 임시 보관소를 쓴다. */

const TREND_MAX_MB=5;                       /* 파일 한 개 최대 용량 */
  ...
  host.innerHTML=
   `<aside class="tsidebar">
      ...
      ${admin?`<div class="tusage" title="게시판에 올라온 파일 용량의 합계입니다">
        ...
        <div class="tun">무료 한도 1GB · 파일 한 개 ${TREND_MAX_MB}MB 까지</div>
      </div>`:''}
    </aside>
    <section class="tmain">
      <div class="tbar">
...
function openTrendForm(post){
  ...
  ov.querySelector('.tdbody').innerHTML=
   `<div class="tform">
      ...
      <div class="tfnote">파일 한 개당 ${TREND_MAX_MB}MB 까지 · PDF · 이미지 · 엑셀 · 워드 · PPT · 영상</div>
  ...
  q('#tfOk').onclick=async()=>{
    ...
    q('#tfOk').disabled=true;q('#tfState').textContent='올리는 중…';
    try{
      ...
      await trendPut(rec,picked,thumbFile,edit);
      ov.remove();
    }catch(e){
...
/* ---------- 저장 ---------- */
async function trendPut(rec,picked,thumbFile,edit){
  ...
  if(trendOn()){
    for(const f of picked){
      ...
      if(error)throw new Error('파일 올리기 실패 — '+error.message);
