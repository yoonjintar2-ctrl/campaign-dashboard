...
/* ---------- 작은 도구들 ---------- */
/* 글 id 는 DB 의 uuid 칼럼에 그대로 들어간다 — 반드시 진짜 UUID 여야 한다 (v67).
   예전에는 'tp' + 36진수였고, 클라우드에 올릴 때 uuid 형식 오류로 저장이 막혔다. */
function trendUid(){
  ...
  return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g,
    c=>c==='x'?r():((Math.floor(Math.random()*4)+8).toString(16)));
}
/* 저장소에 쓸 파일 경로 (v69).
   Supabase Storage 의 키는 **아스키만** 받는다 — 한글이 섞이면 업로드가
   "Invalid key" 로 막힌다. 그래서 경로는 영문·숫자로만 만들고,
   사람에게 보이는 이름(files[].name)은 원래 그대로 둔다.
   내려받을 때는 그 이름을 Supabase 에 따로 알려 준다 (trendFileURL 참고). */
function trendKey(id,name){
  const s=String(name||'');
  const m=/\.([A-Za-z0-9]{1,8})$/.exec(s);
  const ext=m?('.'+m[1].toLowerCase()):'';
  let base=(m?s.slice(0,-m[0].length):s)
    .replace(/[^A-Za-z0-9._-]+/g,'_')      /* 한글·공백·괄호 등은 모두 _ 로 */
    .replace(/_{2,}/g,'_')
    .replace(/^[._-]+|[._-]+$/g,'')
    .slice(0,60);
  if(!base)base='file';                    /* 이름이 전부 한글이면 */
  const rnd=Math.random().toString(36).slice(2,7);
  return `${id}/${Date.now()}${rnd}_${base}${ext}`;
}
...
/* 올린 이미지를 카드 크기로 줄여 썸네일로 쓴다 (원본을 그대로 쓰면 목록이 무거워진다).
   자르는 규칙은 trendFitCard 와 같다 — 세로로 긴 그림은 윗부분이 남는다. */
function trendShrink(file){
  return new Promise(res=>{
    try{
      ...
      fr.onload=()=>{
        const im=new Image();
        im.onload=()=>{try{res(trendFitCard(im,im.naturalWidth||im.width,im.naturalHeight||im.height));}
          catch(e){res('');}};
        im.onerror=()=>res('');
        im.src=fr.result;};
      fr.onerror=()=>res('');
...
/* ---------- 파일 내용으로 썸네일 만들기 (v67) ----------
   PDF 는 첫 장, 영상은 첫 프레임, PPT·워드·엑셀은 파일 안에 들어 있는 미리보기 그림을 쓴다.
   어느 것도 안 되면 그때만 제목으로 표지를 그린다. */
let PDFJS_P=null;
/* CDN 두 곳을 차례로 시도한다 — 사내망에서 한쪽이 막혀 있어도 다른 쪽으로 붙는다.
   둘 다 안 되면 조용히 포기하고 제목 표지로 돌아간다. */
const PDFJS_CDN=[
  'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/|pdf.min.js|pdf.worker.min.js',
  'https://cdn.jsdelivr.net/npm/pdfjs-dist@3.11.174/build/|pdf.min.js|pdf.worker.min.js'];
function loadPdfJs(){
  if(PDFJS_P)return PDFJS_P;
  PDFJS_P=(async()=>{
    if(window.pdfjsLib)return window.pdfjsLib;
    let last=null;
    for(const row of PDFJS_CDN){
      const [base,lib,wk]=row.split('|');
      try{
        await new Promise((res,rej)=>{
          const sc=document.createElement('script');
          sc.src=base+lib;
          const to=setTimeout(()=>{sc.onerror=sc.onload=null;rej(new Error('시간 초과'));},15000);
          sc.onload=()=>{clearTimeout(to);res();};
          sc.onerror=()=>{clearTimeout(to);rej(new Error('불러오기 실패'));};
          document.head.appendChild(sc);});
        if(!window.pdfjsLib)throw new Error('pdfjsLib 없음');
        try{window.pdfjsLib.GlobalWorkerOptions.workerSrc=base+wk;}catch(e){}
        return window.pdfjsLib;
      }catch(e){last=e;}
    }
    throw new Error('pdf.js 를 못 불러왔습니다 — '+((last&&last.message)||''));
  })();
  PDFJS_P.catch(()=>{});          /* 실패해도 콘솔을 더럽히지 않는다 */
  return PDFJS_P;
}
/* 그린 그림을 카드 비율(16:9)에 맞춘다 — 세로로 긴 문서는 **윗부분**을 보여 준다
   (제목이 맨 위에 있기 때문). 가로로 긴 그림은 높이를 채우고 가운데를 보여 준다.
   카드가 object-fit:cover 라서, 여기서 미리 잘라 두어야 보이는 그대로가 저장된다. */
const TREND_TH_W=480,TREND_TH_H=270,TREND_TH_Q=0.72;
function trendFitCard(src,sw,sh){
  const W=TREND_TH_W,H=TREND_TH_H;
  if(!(sw>0&&sh>0))return '';
  const c=document.createElement('canvas');c.width=W;c.height=H;
  const x=c.getContext('2d');
  x.fillStyle='#fff';x.fillRect(0,0,W,H);
  if(sh/sw>H/W){                       /* 세로가 길다 — 폭을 맞추고 윗부분만 */
    x.drawImage(src,0,0,sw,Math.round(sw*H/W),0,0,W,H);
  }else{                               /* 가로가 길다 — 높이를 맞추고 가운데 */
    const cw=Math.round(sh*W/H);
    x.drawImage(src,Math.round((sw-cw)/2),0,cw,sh,0,0,W,H);
  }
  return c.toDataURL('image/jpeg',TREND_TH_Q);
}
async function trendPdfThumb(file){
  const lib=await loadPdfJs();
  const doc=await lib.getDocument({data:await file.arrayBuffer()}).promise;
  const pg=await doc.getPage(1);
  const v1=pg.getViewport({scale:1});
  const vp=pg.getViewport({scale:Math.min(3,960/v1.width)});
  const c=document.createElement('canvas');
  c.width=Math.round(vp.width);c.height=Math.round(vp.height);
  await pg.render({canvasContext:c.getContext('2d'),viewport:vp}).promise;
  return trendFitCard(c,c.width,c.height);
}
function trendVideoThumb(file){
  return new Promise(res=>{
    let url='';
    const done=r=>{try{URL.revokeObjectURL(url);}catch(e){}res(r||'');};
    try{
      url=URL.createObjectURL(file);
      const v=document.createElement('video');
      v.muted=true;v.playsInline=true;v.preload='auto';
      v.onloadeddata=()=>{try{v.currentTime=Math.min(1,(v.duration||2)/4);}catch(e){grab();}};
      const grab=()=>{try{
        if(!v.videoWidth){done('');return;}
        done(trendFitCard(v,v.videoWidth,v.videoHeight));
      }catch(e){done('');}};
      v.onseeked=grab;
      v.onerror=()=>done('');
...
/* PPTX · DOCX · XLSX 는 zip 이다 — 안에 docProps/thumbnail.* 이 있으면 그걸 쓴다 */
async function trendZipThumb(file){
  try{
    ...
    if(eo<0)return '';
    const n=dv.getUint16(eo+10,true);let p2=dv.getUint32(eo+16,true);
    const dec=new TextDecoder();
    for(let k=0;k<n;k++){
      if(dv.getUint32(p2,true)!==0x02014b50)break;
      const method=dv.getUint16(p2+10,true);
      const csize=dv.getUint32(p2+20,true);
      const nl=dv.getUint16(p2+28,true),el=dv.getUint16(p2+30,true),cl=dv.getUint16(p2+32,true);
      const lho=dv.getUint32(p2+42,true);
      const name=dec.decode(buf.subarray(p2+46,p2+46+nl));
      if(/^docProps\/thumbnail\.(jpe?g|png)$/i.test(name)){
        const lnl=dv.getUint16(lho+26,true),lel=dv.getUint16(lho+28,true);
        const st=lho+30+lnl+lel;
        let data=buf.subarray(st,st+csize);
        if(method===8){
          const ds=new DecompressionStream('deflate-raw');
          data=new Uint8Array(await new Response(
            new Blob([data]).stream().pipeThrough(ds)).arrayBuffer());
        }else if(method!==0)return '';
        const type=/\.png$/i.test(name)?'image/png':'image/jpeg';
        return await trendShrink(new File([data],'t',{type}));
      }
      p2+=46+nl+el+cl;
    }
  }catch(e){}
...
/* 붙인 파일들을 앞에서부터 훑어 처음 성공한 것을 쓴다 */
async function trendAutoThumb(files,say){
  for(const f of (files||[])){
    const kind=trendFileKind(f.name);
    try{
      if(kind==='image'){const t=await trendShrink(f);if(t)return t;}
      else if(kind==='pdf'){
        say&&say('PDF 첫 장으로 썸네일 만드는 중…');
        const t=await trendPdfThumb(f);if(t)return t;}
      else if(kind==='video'){
        say&&say('영상 첫 장면으로 썸네일 만드는 중…');
        const t=await trendVideoThumb(f);if(t)return t;}
      else if(['ppt','word','excel'].includes(kind)){
        say&&say('파일 안의 미리보기를 찾는 중…');
        const t=await trendZipThumb(f);if(t)return t;}
    }catch(e){}
  }
  return '';
}
...
/* dl=true 면 "내려받기용" 주소 — 저장소 경로는 아스키로 바꿔 두므로(trendKey)
   원래 파일 이름으로 받게 하려면 Supabase 에 download 이름을 따로 알려 줘야 한다 (v69). */
function trendFileURL(p,i,dl){
  const f=(p.files||[])[i];if(!f)return '';
  if(TREND_BLOBS[p.id+'|'+i])return TREND_BLOBS[p.id+'|'+i];
  if(f.url)return f.url;
  if(f.path&&trendOn()){
    try{const {data}=CLOUD.sb.storage.from(TREND_BUCKET)
        .getPublicUrl(f.path,dl&&f.name?{download:f.name}:undefined);
      return (data&&data.publicUrl)||'';}catch(e){
      try{const {data}=CLOUD.sb.storage.from(TREND_BUCKET).getPublicUrl(f.path);
        return (data&&data.publicUrl)||'';}catch(e2){return '';}}}
  return '';
}
function trendDownload(p,i){
  const f=(p.files||[])[i];if(!f)return;
  const url=trendFileURL(p,i,true);
...
async function trendPut(rec,picked,thumbFile,edit,say){
  ...
  if(trendOn()){
    ...
    for(const f of picked){
      ...
      const path=trendKey(rec.id,f.name);
...
function trendUploadMsg(error,f){
  ...
  /* 저장소 경로에 아스키가 아닌 글자가 섞였을 때 — 평소에는 trendKey 가 미리 걸러 준다 */
  if(/Invalid key/i.test(m))
    return `${f.name} 의 이름을 저장소가 받지 못했습니다. `
      +'파일 이름을 영문·숫자로 바꿔 다시 올려 보시고, 계속 같으면 알려 주세요.';
  if(/already exists|Duplicate/i.test(m))
    return `${f.name} 과(와) 같은 경로가 이미 있습니다. 잠시 뒤 다시 올려 주세요.`;
  return '파일 올리기 실패 — '+m;
}
