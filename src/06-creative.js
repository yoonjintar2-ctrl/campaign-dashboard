...
let RAW_CATALOG=fieldCatalog('dash',f=>!!METRICS[f.k]);
let RAW_DEF={};RAW_CATALOG.forEach(g=>g.cols.forEach(c=>RAW_DEF[c.k]=c));
COLREB.push(()=>{RAW_CATALOG=fieldCatalog('dash',f=>!!METRICS[f.k]);
  RAW_DEF={};RAW_CATALOG.forEach(g=>g.cols.forEach(c=>RAW_DEF[c.k]=c));});
...
function segValues(dim,all){
  if(!dim||dim==='none')return [];
  const fs=factFilter();
  const vals=segSort([...new Set(fs.map(f=>f[dim]))].filter(Boolean),dim,fs);
  if(all)return vals;
  const off=RAW_HIDE[dim]||[];
  return vals.filter(v=>off.indexOf(v)<0);
}
/* 표시 항목 고르기 — 세로·가로 세그먼트를 한 창에서 (체크로 넣고 빼고, 끌어서 순서) */
function openSegPicker(){
  const dims=[];
  if(RAW_SEG!=='none')dims.push({dim:RAW_SEG,axis:'세로'});
  if(RAW_HSEG!=='none'&&RAW_HSEG!==RAW_SEG)dims.push({dim:RAW_HSEG,axis:'가로'});
  if(!dims.length){
    openModal('일자별 상세 효율 — 표시 항목',
      '<div class="hint">세로·가로 세그먼트를 "없음" 이 아닌 값으로 고르면 여기서 항목을 고를 수 있습니다.</div>',
      '<button class="btn primary" data-close>닫기</button>',{w:460});
    return;}
  const lab=k=>(SEG_OPTS.find(s=>s.k===k)||{l:k}).l.replace(/별$/,'');
  const body=dims.map(d=>`<div class="sec gap" style="margin:4px 0 8px;font-size:14px">${d.axis} · ${esc(lab(d.dim))}
      <span class="cnt2" data-cnt="${esc(d.dim)}"></span></div>
    <div class="dplist" data-seglist="${esc(d.dim)}"></div>
    <div style="margin:6px 0 16px"><button class="btn sm" data-sall="${esc(d.dim)}">전체 표시</button>
      <button class="btn sm" data-snone="${esc(d.dim)}">전체 숨김</button></div>`).join('');
  openModal('일자별 상세 효율 — 표시 항목',
    `<div class="hint" style="margin-bottom:10px">체크를 풀면 표에서 빠집니다. 왼쪽을 끌어 순서를 바꿀 수 있습니다.</div>${body}`,
    '<div class="spacer"></div><button class="btn primary" data-close>닫기</button>',{w:560});
  const host=$('modalHost');
  const draw=()=>{
    dims.forEach(({dim})=>{
      const all=segValues(dim,true),off=RAW_HIDE[dim]||[];
      const fs=factFilter();
      const box=host.querySelector(`[data-seglist="${CSS.escape(dim)}"]`);
      ...
      let from=null;
      box.querySelectorAll('.dprow').forEach(row=>{
        ...
        row.addEventListener('dragend',()=>{from=null;
          box.querySelectorAll('.dprow').forEach(r=>r.classList.remove('dragging','dropT','dropB'));});
        ...
        row.addEventListener('drop',e=>{if(from===null||from===row.dataset.k)return;e.preventDefault();
          const r=row.getBoundingClientRect(),after=e.clientY>r.top+r.height/2;
          const list=segValues(dim,true);
          const a=list.indexOf(from);if(a>=0)list.splice(a,1);
          let b2=list.indexOf(row.dataset.k);if(b2<0)b2=list.length;
          list.splice(after?b2+1:b2,0,from);
          RAW_ORDER[dim]=list;from=null;
          renderRaw();draw();try{markDirty();saveLocal();}catch(x){}});});});
    host.querySelectorAll('[data-sall]').forEach(b=>b.onclick=()=>{
      RAW_HIDE[b.dataset.sall]=[];renderRaw();draw();try{markDirty();saveLocal();}catch(e){}});
    host.querySelectorAll('[data-snone]').forEach(b=>b.onclick=()=>{
      RAW_HIDE[b.dataset.snone]=segValues(b.dataset.snone,true);
      renderRaw();draw();try{markDirty();saveLocal();}catch(e){}});};
  draw();
}
...
COLREB.push(()=>{CR_CATALOG=fieldCatalog('dash',f=>!!METRICS[f.k]);
  CR_DEF={};CR_CATALOG.forEach(g=>g.cols.forEach(c=>CR_DEF[c.k]=c));});
...
/* mode
   배열   — 그 필드들 + 이름이 같을 때만 합친다 (게재 히스토리: 표 머리글에 세운 차원 그대로)
...
function mergeCreatives(list,mode){
  ...
  const m=new Map();
  list.forEach(c=>{
    const k=Array.isArray(mode)?mode.map(f=>c[f]).concat(c.name).join(SEP)
      :mode==='name'?c.name
...
function filteredCreatives(){
  ...
  return a;}
/* 게재 히스토리 전용 목록 —
   **표 머리글에 세운 차원(GANTT.rows)만으로 나눈다.**
   머리글이 "매체 · 소재" 뿐이면 광고상품 · 타겟팅이 달라도 같은 매체 안의 같은 이름은
   한 줄로 합쳐 값을 더한다. "매체 구분 없이 비교" 토글의 영향은 받지 않는다. */
function ganttCreatives(){
  const keys=['segment','media','line'];
  /* 소재 이름은 항상 열쇠에 들어가므로 차원 목록에서는 뺀다 */
  const dims=(GANTT.rows||[]).map(r=>r.k).filter(k=>k!=='creative');
  const a=mergeCreatives(CREATIVES.filter(c=>(CR_FILTER.type==='all'||c.type===CR_FILTER.type)
    &&keys.every(k=>FILTER[k]==='all'||c[k]===FILTER[k])),dims);
  const sv=c=>{const b=crAgg(c);return CR_FILTER.sort==='ctr'?b.click/b.imp:b[CR_FILTER.sort];};
  a.sort((x,y)=>(sv(y)||0)-(sv(x)||0));
  return a;}
...
/* ===== 소재 자료함 (캠페인 단위) =====
   예상 효율(라인)을 지웠다 다시 넣으면 소재 레코드는 새로 만들어진다.
   그때마다 올려 둔 이미지·영상이 날아가지 않도록 **이름을 열쇠로** 따로 보관한다.
   같은 이름의 소재가 다시 등록되면 자동으로 다시 붙는다. */
let CR_ASSETS={};
const CR_ASSET_FIELDS=['img','clip','yt','type','ratio','g'];
/* 소재에 붙은 파일·유형을 자료함에 적어 둔다 */
function crAssetSave(c){
  if(!c||!c.name)return;
  const o=CR_ASSETS[c.name]||(CR_ASSETS[c.name]={});
  CR_ASSET_FIELDS.forEach(k=>{if(c[k]!==undefined&&c[k]!=='')o[k]=c[k];});
  o.at=Date.now();
}
/* 자료함에 같은 이름이 있으면 소재에 붙여 준다 (이미 값이 있으면 건드리지 않는다) */
function crAssetApply(c){
  const o=c&&CR_ASSETS[c.name];if(!o)return c;
  CR_ASSET_FIELDS.forEach(k=>{if(!c[k]&&o[k])c[k]=o[k];});
  return c;
}
/* 지금 등록된 소재 전체에 자료함을 다시 붙인다 (문서를 열거나 라인을 새로 넣은 뒤) */
function crAssetRelink(){
  if(typeof CREATIVES==='undefined')return 0;
  let n=0;CREATIVES.forEach(c=>{if(CR_ASSETS[c.name]){crAssetApply(c);n++;}});
  return n;
}
/* 자료함에는 있는데 지금 어느 라인에도 안 붙어 있는 이름 — 삭제를 권한다 */
function crAssetOrphans(){
  const used=new Set((typeof CREATIVES!=='undefined'?CREATIVES:[]).map(c=>c.name));
  return Object.keys(CR_ASSETS).filter(n=>!used.has(n));
}
/* 문서에 담을 자료함 — **지금 소재에 붙어 있는 것은 뺀다.**
   그건 creatives 안에 이미 들어 있고 문서를 열 때 crAssetSave 로 다시 채워지므로,
   같은 이미지를 두 번 담아 문서 용량을 두 배로 키울 필요가 없다. */
function crAssetsForSave(){
  const out={};crAssetOrphans().forEach(n=>{out[n]=CR_ASSETS[n];});return out;
}
function crAssetRename(oldName,newName){
  if(!CR_ASSETS[oldName]||oldName===newName)return;
  CR_ASSETS[newName]={...CR_ASSETS[oldName],...(CR_ASSETS[newName]||{})};
  delete CR_ASSETS[oldName];
}
...
function openCrManage(){
  ...
  /* 자료함에는 남아 있는데 지금 예상 효율 어디에도 없는 이름 —
     예상 효율을 다시 넣을 때 이름이 바뀌었거나 빠진 소재다. 지우기를 권한다. */
  const orphans=crAssetOrphans();
  if(orphans.length){
    h+=`<div class="sec gap" style="margin:18px 0 8px;font-size:14px">매칭 없음
        <span class="cnt2">${orphans.length}개</span></div>
      <div class="hint" style="margin-bottom:8px">올려 둔 이미지·영상은 남아 있지만
        지금 <b>예상 효율에 같은 이름의 소재가 없습니다.</b>
        예상 효율에 그 이름을 다시 넣으면 자동으로 연결되고, 쓸 일이 없다면 지우는 편이 좋습니다.</div>
      <table class="tbl lite" style="background:var(--surface);border-radius:10px;overflow:hidden"><thead><tr>
        <th style="width:52px">미리보기</th><th style="min-width:240px">소재명</th>
        <th style="width:110px">유형</th><th style="min-width:200px">상태</th>
        <th style="width:60px"></th></tr></thead><tbody>`;
    orphans.forEach(n=>{
      const a=CR_ASSETS[n]||{};
      const bg=a.img?`url(&quot;${a.img.slice(0,4)==='data'?a.img:encodeURI(a.img)}&quot;)`:(a.g||crGrad({name:n}));
      h+=`<tr>
        <td><span class="crthumb-sm" style="background-image:${bg}"></span></td>
        <td style="font-weight:700">${esc(n)}</td>
        <td class="hint">${a.type==='video'?'영상':'이미지'}</td>
        <td><span class="badge warn">매칭 없음 · 삭제 권장</span></td>
        <td><button class="btn sm danger" data-adel="${esc(n)}">삭제</button></td></tr>`;});
    h+=`</tbody></table>
      <div style="margin-top:8px"><button class="btn sm danger" id="aDelAll">매칭 없는 소재 모두 삭제</button></div>`;
  }
  ...
  host.querySelectorAll('tr[data-cn]').forEach(tr=>{
    ...
    if(up)up.onchange=e=>{const f=e.target.files&&e.target.files[0];if(!f)return;
      say('줄이는 중…');
      shrinkImage(f,url=>{list.forEach(c=>{c.img=url;c.type='image';crAssetSave(c);});reopen();});};
    ...
    if(vup)vup.onchange=e=>{const f=e.target.files&&e.target.files[0];if(!f)return;
      ...
      encodeVideo(f,say,(r,msg)=>{
        ...
        list.forEach(c=>{c.img=r.poster||'';c.clip=r.clip||'';c.type='video';crAssetSave(c);});
        if(msg)say(msg);
        reopen();});};
    const clr=tr.querySelector('[data-cclr]');
    if(clr)clr.onclick=()=>{list.forEach(c=>{c.img='';c.clip='';});
      /* 자료함에서도 지운다 — 안 그러면 다시 그려질 때 되살아난다 */
      const a=CR_ASSETS[list[0].name];if(a){delete a.img;delete a.clip;}
      reopen();};});
  host.querySelectorAll('[data-adel]').forEach(b=>b.onclick=()=>{
    delete CR_ASSETS[b.dataset.adel];
    try{markDirty();saveLocal();}catch(e){}
    reopen();});
  {const all=host.querySelector('#aDelAll');
   if(all)all.onclick=()=>confirmModal('매칭 없는 소재를 모두 지울까요?',
     '올려 둔 이미지·영상이 사라집니다. 예상 효율에 같은 이름을 다시 넣어도 연결되지 않습니다.',
     ()=>{crAssetOrphans().forEach(n=>delete CR_ASSETS[n]);
       try{markDirty();saveLocal();}catch(e){}
       closeModal();openCrManage();},'모두 삭제');}
  ...
  host.querySelectorAll('[data-cdel]').forEach(b=>b.onclick=()=>{
    ...
    confirmModal(`"${nm}" 소재를 삭제할까요?`,
      ...
      delete CR_ASSETS[nm];                    /* 자료함에서도 완전히 지운다 */
...
function openLightbox(c){
  ...
  if(up)up.onchange=e=>{const f=e.target.files&&e.target.files[0];if(!f)return;
    shrinkImage(f,url=>{
      /* 같은 이름의 소재 전체에 반영 */
      CREATIVES.filter(x=>x.name===c.name).forEach(x=>{x.img=url;crAssetSave(x);});
  ...
  $('lbSave').onclick=()=>{
    ...
    if(y)CREATIVES.filter(x=>x.name===c.name).forEach(x=>{x.yt=ytId(y.value);crAssetSave(x);});
...
function ganttEff(list,SC,keyOf){
  ...
  /* 색의 기준 폭은 **지금 세워 둔 머리글 기준(keyOf)** 안에서 잡는다 (v57).
     예전에는 행(소재) 하나 안에서만 잡아서, 같은 매체의 소재끼리 누가 더 좋은지
     색으로는 알 수 없었다 — 소재마다 자기 최댓값이 제일 진했기 때문이다.
     이제 매체별로 보고 있으면 그 매체 안의 모든 소재·모든 날이 한 척도를 쓴다.
     머리글이 구분이면 구분 안에서, 제품이면 제품 안에서 — 세워 둔 기준을 그대로 따른다. */
  const key=typeof keyOf==='function'?keyOf:()=>'';
  const byGrp={};
  list.forEach(c=>{
    const g=key(c);
    for(let i=SC.i0;i<=SC.i1;i++){const v=val[c.id+'|'+i];
      if(v!==undefined)(byGrp[g]||(byGrp[g]=[])).push(v);}});
  const band={};
  Object.keys(byGrp).forEach(g=>{const a=byGrp[g];
    if(a.length>=2)band[g]={mn:Math.min(...a),mx:Math.max(...a),md:median(a)};});
  const gmn=Math.min(...vals),gmx=Math.max(...vals),gmd=median(vals);
  const EMPTY=cssVar('--acc-soft')||'#dfe4ea';
  /* 값 하나를 색으로 (그룹 척도 안에서). 그룹에 견줄 값이 둘도 없으면 표 전체 기준. */
  const paint=(v,g)=>{
...
function ganttDayVal(c,gi,k){
  const g=m=>(c.daily[m]&&c.daily[m][gi])||0;
  const M=METRICS[k];
  if(!M)return 0;
  if(M.kind!=='rate')return g(k);
  const b={cost:g('cost'),imp:g('imp'),click:g('click'),view:g('view'),
    conv:g('conv'),eng:g('eng'),install:g('install'),rev:g('rev')};
  const v=M.c(b);
  return isFinite(v)&&v>0?v:0;
}
/* 칸 안에 겹쳐 쓰는 아주 작은 숫자 — 열 폭을 넓히지 않도록 짧게 줄인다 */
function ganttTiny(v){
  if(!isFinite(v)||v<=0)return '';
  if(v>=1e8)return Math.round(v/1e8)+'억';
  if(v>=1e4)return (v/1e4).toFixed(v>=1e5?0:1).replace(/\.0$/,'')+'만';
  if(v>=1000)return (v/1000).toFixed(v>=1e4?0:1).replace(/\.0$/,'')+'천';
  return String(Math.round(v));
}
/* 그룹 소계용 가짜 소재 — 그 그룹 소재들의 일별 값을 더해 하나로 만든다 (v57) */
function ganttGroupTotal(cs,name){
  const KEYS=AMET.concat(['cost']);
  const o={id:'gsum|'+name,name,__sub:true,
    lids:[...new Set(cs.map(c=>c.lid))],daily:{}};
  ['segment','media','product','target','line','slot'].forEach(f=>{o[f]=cs[0]?cs[0][f]:'';});
  KEYS.forEach(k=>{
    const a=[];
    cs.forEach(c=>{const s2=c.daily[k]||[];
      for(let i=0;i<s2.length;i++)a[i]=(a[i]||0)+(+s2[i]||0);});
    o.daily[k]=a;});
  return o;
}
...
function renderGantt(){
  ...
  /* ---- 머리 열 너비 — 열마다 **그 열에 실제로 들어가는 글자**에 맞춘다 (v65) ----
     예전에는 폭을 못 박아 두고 남는 가로 폭을 머리 열 하나가 통째로 가져갔다(flexlead).
     그래서 소재 열을 글자 수에 맞추자 이번엔 매체 열이 부풀어 올랐다 — 풍선효과.
     이제 모든 머리 열을 글자 폭으로 재고, 남는 폭은 **날짜 칸**이 고르게 나눠 갖는다. */
  const leadW=(()=>{
    const MIN={segment:66,media:66,product:82,target:82,line:74,creative:104,month:62};
    const MAX=240;
    let cx=null;
    try{
      ...
      cx.font=`${cs.fontWeight||400} ${cs.fontSize||'11.5px'} ${cs.fontFamily||'sans-serif'}`;
    }catch(e){}
    ...
    dims.forEach((d,i)=>{
      ...
      /* 소계 문구에는 썸네일이 없다 — 썸네일 몫까지 더하면 열이 헛되이 넓어진다 */
      if(subText[d])need=Math.max(need,subText[d]+padBase);
      out[d]=Math.round(Math.max(MIN[d]||70,Math.min(MAX,need)));});
    return out;})();
  ...
  /* ---- 머리글 기준 그룹 (소재를 뺀 나머지 차원) ---- */
  const gIdx=dims.map((d,i)=>d==='creative'?-1:i).filter(i=>i>=0);
  const grpKey=vals=>gIdx.map(i=>vals[i]).join(SEP);
  const GK={};rowsData.forEach(r=>{GK[r.c.id]=grpKey(r.vals);});
  /* 색 기준은 지금 세워 둔 머리글 그룹 안에서 잡는다 (v57) */
  const EFF=ganttEff(rowsData.map(r=>r.c),SC,c=>GK[c.id]!==undefined?GK[c.id]:'');
  /* 효율을 못 구하는 경우에만 쓰는 예비 농도 — 같은 그룹의 최댓값 기준 */
  const grpMax={};
  rowsData.forEach(r=>{const g=GK[r.c.id],a=r.c.daily[mk]||[];
    let m=grpMax[g]||0;
    for(let i=SC.i0;i<=SC.i1&&i<ELAPSED;i++){const v=ganttDayVal(r.c,i,mk);if(v>m)m=v;}
    grpMax[g]=m;});
  /* 단가(CPM·CPC·CPV)를 고르면 칸마다 아주 작게 숫자도 얹는다 (v57).
     열 폭은 그대로 두고 넘치는 글자는 잘라 낸다 — 좁은 캠페인에서는 아래에서 통째로 끈다. */
  const RATEM=METRICS[mk]&&METRICS[mk].kind==='rate';
  let h='<thead><tr>';
  /* 머리 열은 전부 글자 폭으로 고정한다 — 어느 한 열이 남는 폭을 가져가지 않는다 (v65) */
  dims.forEach((d,i)=>h+=`<th class="lead" rowspan="2" style="left:${lefts[i]}px;width:${leadW[d]||110}px;min-width:${leadW[d]||110}px;max-width:${leadW[d]||110}px">${(DIMS.find(x=>x.k===d)||{l:d}).l}</th>`);
  ...
  /* sub 를 켜면 소계 줄 — v65 부터 소계 줄의 날짜 칸은 **완전히 비운다**.
     그라데이션도, 집행 일자 막대도, 칸 위 숫자도 없이 옆의 효율 열과 같은 배경색만 남는다.
     (소계는 묶음의 합이라 날짜별 막대가 견줄 대상이 없다 — 숫자는 왼쪽 효율 열에 이미 있다) */
  const dayCells=(c,opt)=>{
    ...
    VD.forEach((d,i)=>{
      ...
      if(sub){out+=`<td class="day plain${i===0?' gsep':''}" data-di="${i}"></td>`;return;}
  ...
  LAY.out.forEach((r,i)=>{
    if(r.kind==='data'){
      ...
      rd.vals.forEach((v,ci)=>{const sp=span[i][ci];if(!sp)return;
        ...
        h+=`<td class="lead ${isCr?'nm':''}" data-lvl="${ci}" data-pk="${esc(rd.vals.slice(0,ci+1).join(SEP))}"`
          +` data-pp="${esc(rd.vals.slice(0,ci).join(SEP))}"${sp>1?` rowspan="${sp}"`:''} style="left:${lefts[ci]}px;width:${leadW[dims[ci]]||110}px;min-width:${leadW[dims[ci]]||110}px;max-width:${leadW[dims[ci]]||110}px">`
    ...
    for(let ci=0;ci<=L;ci++){const sp=span[i][ci];if(!sp)continue;
      h+=`<td class="lead" data-lvl="${ci}"${sp>1?` rowspan="${sp}"`:''}`
        +` style="left:${lefts[ci]}px;width:${leadW[dims[ci]]||110}px;min-width:${leadW[dims[ci]]||110}px;max-width:${leadW[dims[ci]]||110}px">${esc(vals[ci])}</td>`;}
    const nCol=Math.max(dims.length-(L+1),1);
    const li=Math.min(L+1,dims.length-1);
    const wSum=dims.slice(li,li+nCol).reduce((a,d)=>a+(leadW[d]||110),0);
    h+=`<td class="lead sublab" colspan="${nCol}" style="left:${lefts[li]}px;width:${wSum}px;min-width:${wSum}px;max-width:${wSum}px">`
  ...
  t.innerHTML=h+'</tbody>';
  markBlanks(t);
  t.classList.toggle('wnum',!!RATEM);
  /* 남는 가로 폭은 **날짜 칸이 고르게** 나눠 갖는다 (v65).
  ...
  (function fitDays(){
    let w=dayBase;
    try{
      const wrap=t.closest('.gantt-wrap');
      const avail=wrap?wrap.clientWidth:0;
      if(avail>0&&VD.length){
        const room=Math.floor((avail-leadTotal-2)/VD.length);
        if(room>dayBase)w=Math.min(dayCap,room);}
    }catch(e){}
    t.style.setProperty('--gday',w+'px');
    t.style.minWidth=(leadTotal+VD.length*w)+'px';
  })();
  enableRowDrag(t,GANTT,renderGantt);
  wireGanttHover(t,SC,rowsData.map(r=>r.c),subs);
  mountGanttHead(t);
  ...
  if(RATEM)requestAnimationFrame(()=>{try{
    ...
    /* 글자 폭은 캔버스로 실제로 잰다 — 눈대중으로 잡으면 세 자리 숫자가 억울하게 빠진다.
       같은 문자열은 한 번만 재고 기억해 둔다 (칸은 수백 개라도 값의 종류는 몇십 개뿐) */
    const cx=document.createElement('canvas').getContext('2d');
    const ff=getComputedStyle(one).fontFamily||'sans-serif';
    cx.font='700 7.5px '+ff;
    const memo=Object.create(null);
    const wOf=v=>memo[v]!==undefined?memo[v]
...
function wireGanttHover(t,SC,list,subs){
  const paint=(di,on)=>{
  ...
  delegateCellHover(t,'td.day,th.dhd',{
    ...
    move:(td,e)=>{
      ...
      const b=dayEff(c,gi);
      /* v58: 지금 보고 있는 기준(그래프 값)에 해당하는 줄을 팝업에서 도드라지게 한다.
         mk 를 넘기면 GANTT.metric 과 같을 때만 .hi 가 붙는다. */
      const row=(l,v,mk)=>`<div class="r${mk&&mk===GANTT.metric?' hi':''}">`
        +`<span class="l">${l}</span><b>${v}</b></div>`;
      showTip(e.clientX,e.clientY,
        ...
        +row('노출',fmt(b.imp),'imp')+row('클릭',fmt(b.click),'click')+row('조회',fmt(b.view),'view')
        +row('CTR',pct(b.click/b.imp),'ctr')+row('VTR',pct(b.view/b.imp),'vtr')
        +row('CPM',won(b.cost/b.imp*1000),'cpm')+row('CPC',won(b.cost/b.click),'cpc')
        +row('CPV',won(b.cost/b.view),'cpv')
        +row('광고비',won(b.cost),'cost')
  ...
  /* 소계 행도 마우스를 올리면 툴팁이 떠야 한다 — 다만 "이 날 전체" 합계에는 넣지 않는다
     (소계는 이미 그 소재들의 합이라 두 번 더해진다) */
  const byId={};shown.concat((subs||[]).filter(Boolean)).forEach(c=>{byId[c.id]=c;});
  const dayAll=gi=>{
  ...
  const dayHtml=gi=>{
    ...
    const b=dayAll(gi);
    /* 맨 윗줄은 **지금 고른 그래프 값** 을 보여 준다 (v57) —
       CPC 로 보고 있는데 팝업 첫 줄이 소재 수였던 것이 헷갈렸다 */
    const mk2=GANTT.metric,M=METRICS[mk2];
    const mv=M?(M.kind==='rate'?M.c(b):b[mk2]):null;
    const top=(M&&isFinite(mv)&&mv>0)
      ? `<div class="r hi"><span class="l">${M.l}</span><b>${M.f(mv)}</b></div>`:'';
    return head+top
